...
class DiGrafo:
    ...
    def añadir_arista(self,u,v):
        try:
            if self.__matriz_adyacencia[u][v] == 1: #si ya hay una arista entre los nodos ingresados entonces sale un print
                ...
                self.__matriz_adyacencia[u][v] = 1 #se agrega la arista
        except:
    ...
    def eliminar_arista(self,u,v):
        try:
            if self.__matriz_adyacencia[u][v] == 0: #si no hay una arista para eliminar entre los nodos ingresados entonces sale un print
                ...
                self.__matriz_adyacencia[u][v] = 0 #se elimina la arista
        except:
    ...
    def grado_salida(self,u):
        return len(self.adyacentes(u)) #devuelve la cantidad de adyacentes
    def grado_entrada(self,u):
        cont = 0
        for i in range(self.__cant_nodos):
            if self.__matriz_adyacencia[i][u] == 1: #si hay una arista entre el nodo ingresado y algun otro entonces aumenta el contador
                cont+=1
        return cont
    def buscar_fuente(self):
        i = 0
        band = False
        while i < self.__cant_nodos and not band: #mientras no se hayan visitado todos los nodos y no se haya encontrado la fuente
    ...
    def aciclico(self):
        ...
        return aciclico   
    def mostrar(self):#mostrar tipico de matriz
        print("[")
        for i in range(self.__cant_nodos):
