...
class Tabla_Hash_direccionamiento_abierto: # ej1
    ...
    def insertar(self,clave):
        cont = 0
        direction = self.metodo_hash_division(clave) # se hashea la clave
        while self.__lista[direction] is not None and cont != self.__tamaño: #si en la direccion no hay nada y el contador es menor al tamaño de la tabla. este while busca una posicion disponible
            direction = (direction+1) % self.__tamaño #movimiento circular
            cont+=1 #aumento el contador
        if cont != self.__tamaño: #si no llegamos al final de la tabla
            self.__lista[direction] = clave #en la direccion obtenida metemos la clave
    ...
    def insertar(self,clave):
        ...
        if self.__contadores[direction] < self.__capacidad_buckets:
            ...
            if aux < self.__tamaño-1:
                ...
                print(f"en overflow la clave {clave}\n")
    def buscar(self,clave):
        """direction = self.metodo_extraccion(clave)
        i = 0
        cont = 0
        band = False
        while i < self.__capacidad_buckets and not band:
            if (self.__lista[direction][i]) == float(clave):
        ...
        if not band:
            ...
            aux = self.__direccion_overflow
            print(aux)
            while aux < self.__tamaño-1 and not band:
                while i < self.__capacidad_buckets and not band:
                    print(self.__lista[aux][i], float(clave))
                    if self.__lista[aux][i] == float(clave):
                        print(f"Se encontró la clave {clave} en {cont} comparaciones\n")
                        band = True
                    else:
                        cont+=1
        ...
        j= 0
        band = False
        while i < self.__tamaño-1 and not band:
            while j < self.__capacidad_buckets and not band:
                print(self.__lista[i][j], float(clave))
                if self.__lista[i][j] == float(clave):
                    print(f"Se encontró la clave {clave} en {cont} comparaciones\n")
                    band = True
                else:
                    cont+=1
