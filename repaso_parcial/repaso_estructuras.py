...
class Lista_sec_ord:
    ...
    def insertar(self,dato):
        if self.__cant_elementos == self.__max_elementos: #si está llena no se puede insertar mas nada
            ...
            pos = 0
            while pos < self.__cant_elementos and self.__elementos[0] <= dato: #evalua si la posicion es menor a la cantidad de elementos y el dato de esa posicion es menor o igual al dato
...
class Lista_enc_ord:
    ...
    def insertar(self,dato):
        ...
        if self.vacia() or self.__cabeza.obtener_dato() >= dato: #si la lista está vacia o el dato de la cabeza es mayor o igual al que queremos ingresar entonces el nuevo lo definimos como cabeza (tambien se pone el igual porque no tiene entrar al bucle si es lo mismo xd)
            ...
            aux = self.__cabeza
            while aux is not None and aux.obtener_dato() < dato: #entra si el nodo no es none y aparte su dato es menor al que queremos ingresar
